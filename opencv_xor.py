...
ann.setTrainMethod(cv2.ml.ANN_MLP_BACKPROP, 0.0001)
ann.setActivationFunction(cv2.ml.ANN_MLP_SIGMOID_SYM)
ann.setTermCriteria((cv2.TERM_CRITERIA_COUNT | cv2.TERM_CRITERIA_EPS, 1, 0.001 ))

input_array = np.array([[1.0, 1.0]], dtype=np.float32)
output_array = np.array([[1.0]], dtype=np.float32)
